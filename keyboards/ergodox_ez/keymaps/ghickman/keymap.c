#include QMK_KEYBOARD_H
#include "debug.h"
#include "action_layer.h"
#include "version.h"

#define BASE 0 // default layer
#define MDIA 1 // media keys

const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
/* Keymap 0: Basic layer
 *
 * ,-----------------------------------------------------.           ,--------------------------------------------------.
 * | Esc       |   1  |   2  |   3  |   4  |   5  |  F1  |           |  F4  |   6  |   7  |   8  |   9  |   0  |   [    |
 * |-----------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * | Tab       |   Q  |   W  |   E  |   R  |   T  |  \   |           |   '  |   Y  |   U  |   I  |   O  |   P  |   ]    |
 * |-----------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | LCtrl|Esc |   A  |   S  |   D  |   F  |   G  |------|           |------|   H  |   J  |   K  |   L  |   ;  | RCtrl  |
 * |-----------+------+------+------+------+------|  =   |           |   -  |------+------+------+------+------+--------|
 * | LShift    |   Z  |   X  |   C  |   V  |   B  |      |           |      |   N  |   M  |   ,  |   .  |   /  | RShift |
 * `-----------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   |         |   `  |      | LAlt | LGui |                                       | LGui | Left |  Up  | Down | Right |
 *   `-------------------------------------'                                       `-----------------------------------'
 *                                           ,-------------.       ,-------------.
 *                                           |  F6  |  F5  |       |  F8  |  F7  |
 *                                    ,------|------|------|       |------+------+------.
 *                                    |      |      | Home |       | End  |      |      |
 *                                    |Space |Enter |------|       |------| BkSp | Space|
 *                                    |      |      |  +L1 |       | +L1  |      |      |
 *                                    `--------------------'       `--------------------'
 */
// If it accepts an argument (i.e, is a function), it doesn't need KC_.
// Otherwise, it needs KC_*
[BASE] = LAYOUT_ergodox(  // layer 0 : default
        // left hand
        KC_ESC,         KC_1,      KC_2,   KC_3,     KC_4,     KC_5,   KC_F1,
        KC_TAB,         KC_Q,      KC_W,   KC_E,     KC_R,     KC_T,   KC_BSLASH,
        CTL_T(KC_ESC),  KC_A,      KC_S,   KC_D,     KC_F,     KC_G,
        KC_LSFT,        KC_Z,      KC_X,   KC_C,     KC_V,     KC_B,   KC_EQUAL,
        KC_NO,          KC_GRAVE,  KC_NO,  KC_LALT,  KC_LGUI,
                                                               KC_F6,  KC_F5,
                                                                       KC_HOME,
                                                 KC_SPACE,  KC_ENTER,  MO(1),
        // right hand
        KC_F4,     KC_6,   KC_7,     KC_8,     KC_9,    KC_0,       KC_LBRACKET,
        KC_QUOTE,  KC_Y,   KC_U,     KC_I,     KC_O,    KC_P,       KC_RBRACKET,
                   KC_H,   KC_J,     KC_K,     KC_L,    KC_SCOLON,  KC_RCTRL,
        KC_MINUS,  KC_N,   KC_M,     KC_COMM,  KC_DOT,  KC_SLASH,   KC_RSFT,
                           KC_LGUI,  KC_LEFT,  KC_UP,   KC_DOWN,    KC_RIGHT,
        KC_F8,    KC_F7,
        KC_END,
        MO(1),  KC_BSPACE,  KC_SPACE
    ),
/* Layer 1: Media Keys
 *
 * ,--------------------------------------------------.           ,--------------------------------------------------.
 * | TRNS   |  F1  |  F2  |  F3  |  F4  |  F5  |  F6  |           |  F7  |  F8  |  F9  |  F10 |  F11 |  F12 |        |
 * |--------+------+------+------+------+-------------|           |------+------+------+------+------+------+--------|
 * | TRNS   |      |      |      |      | VolU |      |           |      | VolD |      |      |      |      |        |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | TRNS   |      |      | Mute | F13  |      |------|           |------|      | Prev |Play/P| Next |      |  TRNS  |
 * |--------+------+------+------+------+------|      |           |      |------+------+------+------+------+--------|
 * | TRNS   |      |      |      |      |      |      |           |      |      |      |      |      |      |  TRNS  |
 * `--------+------+------+------+------+-------------'           `-------------+------+------+------+------+--------'
 *   | TRNS | TRNS | TRNS | TRNS | TRNS |                                       | TRNS | TRNS | TRNS | TRNS | TRNS |
 *   `----------------------------------'                                       `----------------------------------'
 *                                        ,-------------.       ,-------------.
 *                                        | TRNS | TRNS |       | TRNS | TRNS |
 *                                 ,------|------|------|       |------+------+------.
 *                                 |      |      |      |       |      |      |      |
 *                                 | TRNS | TRNS |------|       |------| DEL  | TRNS |
 *                                 |      |      |  L0  |       |  L0  |      |      |
 *                                 `--------------------'       `--------------------'
 */

// Media
[MDIA] = LAYOUT_ergodox(
       // left hand
       KC_TRNS,  KC_F1,    KC_F2,    KC_F3,    KC_F4,    KC_F5,      KC_F6,
       KC_TRNS,  KC_NO,    KC_NO,    KC_NO,    KC_NO,    KC__VOLUP,  KC_NO,
       KC_TRNS,  KC_NO,    KC_NO,    KC__MUTE, KC_F13,   KC_NO,
       KC_TRNS,  KC_NO,    KC_NO,    KC_NO,    KC_NO,    KC_NO,      KC_NO,
       KC_TRNS,  KC_TRNS,  KC_TRNS,  KC_TRNS,  KC_TRNS,
                                                         KC_TRNS,    KC_TRNS,
                                                                     KC_NO,
                                              KC_TRNS,   KC_TRNS,    KC_TRNS,
       // right hand
       KC_F7,    KC_F8,        KC_F9,    KC_F10,   KC_F11,   KC_F12,   KC_NO,
       KC_NO,    KC__VOLDOWN,  KC_NO,    KC_NO,    KC_NO,    KC_NO,    KC_NO,
                 KC_NO,        KC_MRWD,  KC_MPLY,  KC_MFFD,  KC_NO,    KC_TRNS,
       KC_NO,    KC_NO,        KC_NO,    KC_NO,    KC_NO,    KC_NO,    KC_TRNS,
                               KC_TRNS,  KC_TRNS,  KC_TRNS,  KC_TRNS,  KC_TRNS,
       KC_TRNS,  KC_TRNS,
       KC_NO,
       KC_TRNS,  KC_DELETE,  KC_TRNS
),
};

const uint16_t PROGMEM fn_actions[] = {
    [1] = ACTION_LAYER_TAP_TOGGLE(MDIA)                // FN1 - Momentary Layer 1 (Media)
};

const macro_t *action_get_macro(keyrecord_t *record, uint8_t id, uint8_t opt)
{
  // MACRODOWN only works in this function
      switch(id) {
        case 0:
        if (record->event.pressed) {
          register_code(KC_RSFT);
        } else {
          unregister_code(KC_RSFT);
        }
        break;
      }
    return MACRO_NONE;
};

// Runs just one time when the keyboard initializes.
void matrix_init_user(void) {

};

// Runs constantly in the background, in a loop.
void matrix_scan_user(void) {

    uint8_t layer = biton32(layer_state);

    ergodox_board_led_off();
    ergodox_right_led_1_off();
    ergodox_right_led_2_off();
    ergodox_right_led_3_off();
    switch (layer) {
      // TODO: Make this relevant to the ErgoDox EZ.
        case 1:
            ergodox_right_led_1_on();
            break;
        case 2:
            ergodox_right_led_2_on();
            break;
        default:
            // none
            break;
    }

};
